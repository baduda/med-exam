...
- `mode`     — "text" (default) if the PDF has a usable text layer, "image" if
               questions must be generated by reading page images. See below.

Two ingestion modes
-------------------
Most books carry (or can be OCR'd into) a text layer, so `extract.py` pulls
text and `chunk.py` cuts word windows. One book cannot: the Periodontologia
source is a set of phone photos at ~96 dpi with curved lines and a finger in
frame, where Tesseract produces ~20% garbled tokens against a <2% gate. For
`mode: "image"` books the pipeline renders page images instead, and the
generating agent reads those images directly.
"""
import fnmatch
import unicodedata
